...
RTEMS_QUAL = False
# Note: 'qual-only' can currenlty not build for ZynqMP
...
INHERIT = $(ZYNQMP_BSP)
RTEMS_BUILD_LABEL = $(ZYNQMP_ARCH)/$(ZYNQMP_BSP)/qual-only-coverage
...
tools-zynqmp: TOOLS_ARCH = $(ZYNQMP_ARCH)
tools-zynqmp: tools
.PHONY: tools-zynqmp

bsp-zynqmp: $(ZYNQMP_INI)
	uv run ./waf configure "--rtems-tools=$(TOOLS_PREFIX)" "--prefix=$(ZYNQMP_PREFIX)"
	uv run ./waf
	uv run ./waf install
.PHONY: bsp-zynqmp
...
clean-zynqmp:
	rm -rf $(ZYNQMP_INI) $(ZYNQMP_PREFIX) build
.PHONY: clean-zynqmp

distclean-zynqmp: clean-zynqmp
	rm -rf config-cache config-tools src tools
.PHONY: distclean-zynqmp
